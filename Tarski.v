...
Section Tarski.
  ...
  Section Substs.
    ...
    Lemma sat_ext rho xi phi :
      (forall x, rho x = xi x) -> rho ⊨_FOL phi <-> xi ⊨_FOL phi.
    Proof.
    ...
    Lemma sat_comp rho xi phi :
      rho ⊨_FOL (subst_form xi phi) <-> (xi >> eval rho) ⊨_FOL phi.
    Proof.
    ...
    Lemma sat_subst rho sigma phi :
      (forall x, eval rho (sigma x) = rho x) -> rho ⊨_FOL phi <-> rho ⊨_FOL (subst_form sigma phi).
    Proof.
  ...
  Section Vector.
    ...
    Lemma split_merge_eq phi N sigma :
      (first_of N sigma ∗ rest_of N sigma) ⊨_FOL phi <-> sigma ⊨_FOL phi. 
    Proof.
    ...
    Lemma vec_exists n phi rho :
      rho ⊨_FOL (exist_times n phi) <-> exists v : vec D n, v ∗ rho ⊨_FOL phi.
    Proof.
      ...
      - intros rho [v H]. exists (Vector.hd v). apply IHn. exists (tl v).
        change (Vector.cons _ (Vector.hd v) _ (tl v) ∗ rho ⊨_FOL phi ).
    ...
    Lemma vec_join_shiftin N phi (v : vec D (S N)) (rho sigma : nat -> D) (x : D) :
      (x .: v ∗ rho) ⊨_FOL phi -> (join (Vector.shiftin x v) rho ⊨_FOL phi <-> join (Vector.shiftin x v) sigma ⊨_FOL phi) -> (x .: v ∗ sigma) ⊨_FOL phi.
    Proof.
      intros H' H.
    ...
    Lemma vec_change N phi (v : vec D (S N) ) rho sigma :
      max_form phi <= N -> v∗rho ⊨_FOL phi <-> v∗sigma ⊨_FOL phi.
    Proof.
  ...
  Section Top.
    ...
    Definition U phi := fun x => forall rho, (x.:rho) ⊨_FOL phi.

    Definition inters {J} (Phi : J -> form) := fun x => forall rho i, (x.:rho) ⊨_FOL Phi i.
    Definition union {J} (Phi : J -> form) := fun x => forall rho, exists i, (x.:rho) ⊨_FOL Phi i.
...
Notation "rho ⊨_FOL phi" := (sat _ rho phi) (at level 20).
