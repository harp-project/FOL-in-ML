...
(* Lemma (up_ML (upn_ML n f)) = (upn_ML n (up_ML f)) *)
...
Lemma up_up_eliminate_term : forall x f,
  convert_term (up_ML f) x`[↑] = convert_term f x.
Proof.
- induction x; intros.
  + simpl. destruct (shift_ML f x ) eqn:P.
    * destruct (f x) eqn:D.
      -- unfold shift_ML in P. rewrite D in P. inversion P. 
      -- unfold shift_ML in P. rewrite D in P. discriminate P.
...
Proof.
  induction φ; intros.
  - reflexivity.
  - simpl. rewrite Vector.map_map.
Qed.

Definition fura f x:= fun n => match n with | 0 => x | S n => (up_ML f) n end.
...
Compute (evar_quantify 5 0 (convert_term convertf ($5)) = convert_term (up_ML convertf) ($5)).
(* evar_quantify (convert_term (up_ML convertf) t) 0 (convert_form convertf Φ[t..]) = convert_form (up_ML convertf) Φ *)
