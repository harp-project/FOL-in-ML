...
Section ND_def.
  ...
  Reserved Notation "A ⊢_MSFOL phi" (at level 61).
  ...
  | II {p} A phi psi : phi::A ⊢_MSFOL psi -> A ⊢_MSFOL phi --> psi
  | IE {p} A phi psi : A ⊢_MSFOL phi --> psi -> A ⊢_MSFOL phi -> A ⊢_MSFOL psi
  | AllI {p} A phi : map (subst_form ↑) A ⊢_MSFOL phi -> A ⊢_MSFOL ∀ phi
  | AllE {p} A t phi : A ⊢_MSFOL ∀ phi -> A ⊢_MSFOL phi[t..]
  | ExI {p} A t phi : A ⊢_MSFOL phi[t..] -> A ⊢_MSFOL ∃ phi
  | ExE {p} A phi psi : A ⊢_MSFOL ∃ phi -> phi::(map (subst_form ↑) A) ⊢_MSFOL psi[↑] -> A ⊢_MSFOL psi
  | Exp {p} A phi : prv p A ⊥ -> prv p A phi
  | Ctx {p} A phi : phi el A -> A ⊢_MSFOL phi
  | CI {p} A phi psi : A ⊢_MSFOL phi -> A ⊢_MSFOL psi -> A ⊢_MSFOL phi ∧ psi
  | CE1 {p} A phi psi : A ⊢_MSFOL phi ∧ psi -> A ⊢_MSFOL phi
  | CE2 {p} A phi psi : A ⊢_MSFOL phi ∧ psi -> A ⊢_MSFOL psi
  | DI1 {p} A phi psi : A ⊢_MSFOL phi -> A ⊢_MSFOL phi ∨ psi
  | DI2 {p} A phi psi : A ⊢_MSFOL psi -> A ⊢_MSFOL phi ∨ psi
  | DE {p} A phi psi theta : A ⊢_MSFOL phi ∨ psi -> phi::A ⊢_MSFOL theta -> psi::A ⊢_MSFOL theta -> A ⊢_MSFOL theta
  | Pc A phi psi : prv class A (((phi --> psi) --> phi) --> phi)
  where "A ⊢_MSFOL phi" := (prv _ A phi).

  Arguments prv {_} _ _.

  Hint Constructors prv : core.

  Notation "A ⊢_MSFOL phi" := (prv A phi) (at level 61).
  ...
  Section ShiftContext.
    ...
    Lemma switch_conj_imp alpha beta phi A : A ⊢_MSFOL alpha ∧ beta --> phi <-> A ⊢_MSFOL alpha --> beta --> phi.
    Proof.
      split; intros H.
    ...
    Lemma switch_imp A alpha phi : A ⊢_MSFOL alpha --> phi <-> alpha::A ⊢_MSFOL phi.
    Proof.
      split.
    ...
    Lemma conj_comm A alpha beta phi : alpha ∧ beta :: A ⊢_MSFOL phi -> beta ∧ alpha :: A ⊢_MSFOL phi.
    Proof.
    ...
    Lemma shift_context A : forall phi, A ⊢_MSFOL phi <-> nil ⊢_MSFOL (Conj A) --> phi.
    Proof.
      induction A.
      - split; intros H.
...
Notation "A ⊢_MSFOL phi" := (prv A phi) (at level 30).
...
Section Soundness.
  ...
  Definition valid A phi :=
    forall D (I : interp D) rho, (forall Phi, Phi el A -> rho ⊨_FOL Phi) -> rho ⊨_FOL phi.
  ...
  Proof.
    ...
    - edestruct IHprv1 as [d HD]; eauto.
      assert (H' : forall psi, phi = psi \/ psi el map (subst_form ↑) A -> (d.:rho) ⊨_FOL psi).
  ...
  Definition tvalid T phi :=
    forall D (I : interp D) rho, (forall phi, phi t∈ T -> rho ⊨_FOL phi) -> rho ⊨_FOL phi.
